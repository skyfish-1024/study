...
func main() {
	...
	//发布
	r.GET("article/publish", func(c *gin.Context) {
	...
	r.POST("blog/login", func(c *gin.Context) {
		...
		if err != nil {
			...
			SetToken(username)
			c.Redirect(http.StatusMovedPermanently, "http://127.0.0.1:9090/home")
		}
	})
	//注册
	...
	//发布文章，有个Jwttoken（）中间件
	r.POST("/publish",Jwttoken(), func(c *gin.Context) {
		...
		if err!=nil{
			...
			c.HTML(http.StatusOK, "/home", blogHome)
		}
	})
	//删除文章
